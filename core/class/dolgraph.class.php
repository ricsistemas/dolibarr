 ...
 *	\version    $Id: dolgraph.class.php,v 1.6 2010/11/22 09:18:54 eldy Exp $
 *
 *	Usage:
...
class DolGraph
{
	...
	function draw_artichow($file)
	{
		...
		if (isset($this->title))
		{
			...
			//CHANGED FONT FROM "Tuffy" to "DejaVuSans" so that we can see hebrew!! AMIT MILSTEIN EYAL LEVIN
			$graph->title->setFont(new DejaVuSans1(10));
		}
		...
		while ($i < $nblot)
		{
			...
			$group->legend->setTextFont(new DejaVuSans1(10)); // This is to force Artichow to use awFileFontDriver to
		...
		$group->axis->bottom->label->setFont(new DejaVuSans1(7));
