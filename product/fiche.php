 ...
 *  \version    $Id: fiche.php,v 1.360.2.1 2011/01/23 20:08:10 hregis Exp $
 */
...
if ($_GET["action"] == 'create' && ($user->rights->produit->creer || $user->rights->service->creer))
{
	...
	if (empty($usecanvas))
	{
		...
		{
			// Weight
			print '<tr><td>'.$langs->trans("Diameter").'</td><td>';
			...
			print '<input name="size" size="25" value="'.$_POST["size"].'">';
...
if ($_GET["id"] || $_GET["ref"])
{
	...
	if ($_GET["action"] == 'edit' && ($user->rights->produit->creer || $user->rights->service->creer))
	{
		if (empty($usecanvas))
		{
			...
			{
				// Weight
				print '<tr><td>'.$langs->trans("Diameter").'</td><td colspan="2">';
				...
				print '<input name="size" size="25" value="'.$product->length.'"> ';
...
llxFooter('$Date: 2011/01/23 20:08:10 $ - $Revision: 1.360.2.1 $');
