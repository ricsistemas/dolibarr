...
if ($mode == 'search')
{
	...
	if ($result)
	{
		if ($db->num_rows($result) == 1)
		{
			...
			$socid = $obj->rowid;
			header("Location: ".DOL_URL_ROOT."/societe/soc.php?socid=".$socid); // CHANGE AMIT EYAL TO LINK COMPANY DIRECTLY TO NEW ORDER!
			//header("Location: ".DOL_URL_ROOT."/societe/soc.php?socid=".$socid."&action=create"); //http://localhost/dolibarr/commande/fiche.php?socid=14&action=create
			exit;
		}
		$db->free($result);
	}
...
if ($resql)
{
	...
	print '<td class="liste_titre" colspan="10" align="center">&nbsp;</td>';
	print "</tr>\n";
	...
	*/
	...
	while ($i < min($num,$conf->liste_limit))
	{
		...
		{
	  		print "<a href=\"".DOL_URL_ROOT."/comm/fiche.php?socid=".$obj->rowid."\">".$langs->trans("CustomerMakeOrder")."</a>\n";
		}
