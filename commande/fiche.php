 ...
 *	\version    $Id: fiche.php,v 1.474.2.1 2011/01/29 16:30:41 hregis Exp $
 */
...
require(DOL_DOCUMENT_ROOT."/pbx.php");
		...
		if ($result >= 0)
		{
			...
			else
			{
				...
				if ($result > 0)
				{
					...
					//THIS IS WHERE WE MAKE A PDF FOR DRAFT AMIT EYAL
					//commande_pdf_create($db, $object, $object->modelpdf, $outputlangs);
					makeDraft($comid);
...
if ($_REQUEST['action'] == 'confirm_validate' && $_REQUEST['confirm'] == 'yes' && $user->rights->commande->valider)
{
	...
	if ($result	>= 0)
	{
		...
		//commande_pdf_create($db, $object, $object->modelpdf, $outputlangs);
		//AMIT EYAL ADD pbx.php
		makeDraft($comid); //Making pdf when order is changed to הזמנה פתוחה
	}
}
...
llxFooter('$Date: 2011/01/29 16:30:41 $ - $Revision: 1.474.2.1 $');
?>
