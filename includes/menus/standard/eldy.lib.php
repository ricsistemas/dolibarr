 ...
 *  \version	$Id: eldy.lib.php,v 1.20.2.2 2011/02/06 23:57:44 eldy Exp $
 */
...
function print_eldy_menu($db,$atarget,$type_user)
{
	...
	if ($conf->societe->enabled || $conf->fournisseur->enabled)
	{
		...
		|| ($conf->fournisseur->enabled && $user->rights->fournisseur->lire))
		{
			...
			print_text_menu_entry($langs->trans("ThirdPartiesHebNew"));
			print '</a>';
			print_end_menu_entry();
		}
	...
	if ($conf->societe->enabled)
	{
		...
		if($user->rights->societe->lire || $user->rights->societe->contact->lire)
		{
			...
			print '<a class="tmenuimage" href="'.DOL_URL_ROOT.'/commande/index.php?mainmenu=commercial&leftmenu=orders"'.($atarget?" target=$atarget":"").'>';
			
			print '<div class="'.$id.' '.$idsel.'"><span class="'.$id.' tmenuimage" id="'.$id.'"></span></div>';
			print '</a>';
			print '<a '.$classname.' id="mainmenua_'.$idsel.'" href="'.DOL_URL_ROOT.'/commande/index.php?mainmenu=commercial&leftmenu=orders"'.($atarget?" target=$atarget":"").'>';
...
function print_left_eldy_menu($db,$menu_array_before,$menu_array_after)
{
    ...
    if ($mainmenu)
    {
        ...
        if ($mainmenu == 'companies')
        {
            ...
            {
                $langs->load("companies");
                $newmenu->add("/comm/clients.php?leftmenu=customers", $langs->trans("ThirdParty"), 0, $user->rights->societe->lire);
            ...
            {
                $langs->load("commercial");
                $newmenu->add("/comm/clients.php?leftmenu=customers", $langs->trans("ListCompaniesShortHeb"), 1, $user->rights->societe->lire);
